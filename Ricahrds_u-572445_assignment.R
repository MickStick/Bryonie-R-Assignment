...
WiiUScores <- videogames[ videogames$Platform == 'WiiU' , names(videogames) == 'Platform' | names(videogames) == 'Critic_Score' | names(videogames) == 'User_Score' | names(videogames) == 'Critic_Count' | names(videogames) == 'User_Count']

# str(WiiUScores)

answer5 <- ggplot() + 
    geom_point(data = videogames, mapping = aes(y = Critic_Score, x = User_Score, color = Critic_Count, size = User_Count)) + 
    labs(title="WiiU Critic Scores by User Scores", x="Users' Scores", y="Critics' Scores", color="Critic Count", size = "User Count") + 
    scale_color_continuous(low = "white", high = "black")
answer5

## Exercise 6 -----------------------------------------------------------------

videogameRatings <- videogames[, names(videogames) == 'Name' | names(videogames) == 'Rating']
...
answer6 = ggplot(data = videogameRatings, mapping = aes( x = Rating)) + 
            geom_bar() + 
            scale_x_discrete(limits = c("E", "E10+", "T", "M"), labels = c("Everyone", "Everyone 10+", "Teen", "Mature")) + 
            labs( x = 'Ratings', y = 'Number of Games', title = 'Number of Games per Rating')
# answer6
...
answer7 <- ggplot(data = UbiAct, mapping = aes( x = Publisher, fill = Genre)) + geom_bar(position = "dodge") + labs( y = 'Genre', x = 'Number of Publishes', title = "Number of Publishes per Genre for 'Ubisoft' and 'Activision'")
# answer7
...
legoGames <- videogames[grepl('LEGO', videogames$Name, fixed=TRUE) , names(videogames) == 'Name' | names(videogames) == 'User_Score' | names(videogames) == 'Global_Sales'] %>% 
    group_by(Name) %>% summarize(Global_Sales = sum(na.omit(Global_Sales)), User_Score = sum(na.omit(User_Score))) #legoGames[!duplicated(legoGames$Name), ]
